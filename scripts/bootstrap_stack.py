...
def main() -> int:
    ...
    wechat_cwd, wechat_env = install_service_without_env_gate(
        services["wechat_exporter"],
        global_env,
        overrides,
        update=not args.no_update,
    )
    install_service_without_env_gate(
        services["mediacrawler_xhs"],
        global_env,
        overrides,
        update=not args.no_update,
    )
    ...
    try:
        ...
        if not args.skip_xhs_login:
            ...
            if not xhs_cookie:
                ...
                run_login_script("xhs_terminal_login.py")
    finally:
    ...
    manage_services.command_up(update=not args.no_update)
    return 0
